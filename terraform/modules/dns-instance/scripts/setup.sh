...
aws s3 cp s3://${config_bucket}/vector/vector.yaml /home/ubuntu/vector.yaml
...
# Download the `dns-server` configuration and get it running
aws s3 cp s3://${config_bucket}/${config_key} /home/ubuntu
aws s3 cp s3://${config_bucket}/dns-server/blocklist.txt /home/ubuntu

# Allow the `ubuntu` user to run `docker` commands (for SSH access)
sudo usermod -aG docker ubuntu

sudo docker run -d -v /home/ubuntu:/etc/dns-server -p 853:853 alexanderjackson/dns-server:${tag} -- --config /etc/dns-server/config.yaml
